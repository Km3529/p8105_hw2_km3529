...
This dataset contains information on the New York City subway system. It includes data on station lines, station coordinates, entrance types, and ADA accessibility. It also provides route name information, as many stops have many lines.  
...
The data are not tidy because there are multiple columns describing route names. A tidyier way to present this would be to have a different row for each route number. 
...
the SNP looked at betweent he years of 

There are `r nrows(final_df)` rows and `r ncol(final_df)`. 
